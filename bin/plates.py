...
all = pd.read_csv("probs.txt",header=None,delimiter="\t",names=["plate","well"])

all['row']=all['well'].apply(lambda x:ord(x[0])-ord('A'))
all['col']=all['well'].apply(lambda x:int(x[2:4]))
...
def parseArguments():
    ...
    parser.add_argument('--samplesheet', type=str, metavar='samplesheet',require=True),
