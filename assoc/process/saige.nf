...
process saige_computed_variance{ 
   ...
   script :
     ...
     ${params.saige_bin_fitmodel} \
        ...
        --minMAFforGRM=${params.cut_maf_rel} $binpheno \
...
process doSaige{
   ...
   publishDir "${params.output_dir}/saige/log/", overwrite:false, mode:'copy', pattern: "*.report"
    output :
      tuple val(our_pheno),path("$output"),emit : saige
      path("*.report")
    script :
...
process doSaigeBgen{
   ...
   publishDir "${params.output_dir}/saige/log/", overwrite:false, mode:'copy', pattern: "*.report"
    output :
      tuple val(pheno), path("$output"),emit :saige
      path("*.report")
    script :
     ...
     Chro=(Chro=="23") ? "X": "$Chro"
     Loco = (params.loco==1) ? " --LOCO=TRUE " : " --LOCO=FALSE "
     imputed=(params.imputed_data==1) ? "--is_imputed_data=TRUE --impute_method=${params.saige_impute_method} " : '--is_imputed_data=FALSE' 
     moredetail=" --is_output_moreDetails=TRUE " 
     """
      ${params.saige_bin_spatest} \
        --bgenFile=$bgen    \
	...
	--SAIGEOutputFile=$output $imputed $Loco $moredetail  ${params.saige_otheropt_step2}   ${params.saige_otheropt}  --maxMissing ${params.cut_geno}
      cp .command.sh "${pheno}"_saige_step2.cmd.report
      cp .command.log "${pheno}"_saige_step2.log.report
      cp .command.err "${pheno}"_saige_step2.err.report
     """
}
...
process doSaigePlink{
    ...
    output :
      tuple val(pheno),file("$output"),emit : saige
      path("*.report")
   publishDir "${params.output_dir}/saige/log/", overwrite:false, mode:'copy', pattern: "*.report"
    script :
...
workflow saige{
 ...
 if(params.vcf!='' || params.vcf_list!=''){
       doSaige(saige_computed_variance.out.var.combine(vcf))
        ...
        doSaigePlink(saige_computed_variance.out.var.combine(listchro).combine(plink))
        lsumstat=doSaigePlink.out.saige
 }
   merge_sumstat(channel.from('saige').combine(lsumstat.groupTuple()))
}
