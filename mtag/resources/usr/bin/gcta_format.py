...
def parseArguments():
    ...
    parser.add_argument('--file_updaters',type=str,required=False,help="", default=None)
...
if args.file_updaters :
#    10:10000030	C	A	10	10000030	rs573722980
  bim=pd.read_csv(args.file_updaters, header=None,delim_whitespace=True)
  del result['SNP']
  bim.columns=['SNPOld','A1_bim','A2_bim','CHR','BP','SNP']
  bim['CHR']=bim['CHR'].astype(str)
  del bim['SNPOld']
  result=result.merge(bim,left_on=['chro',"bp"],right_on=["CHR","BP"])
  balise=((result['A1']==result['A1_bim']) & (result['A2']==result['A2_bim'])) | ((result['A1']==result['A2_bim']) & (result['A2']==result['A1_bim']))
  result=result[NewHead]
  result=result.loc[balise]
