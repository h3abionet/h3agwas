...
def getInputChannels() {
    ...
    return [
        ...
        channel.fromPath(params.case_control)]
}
...
process removeSkewSnps {
  ...
  script:
    base = bed.baseName
    output = params.cohortName+".filteredSamples"
    """
...
process collectPlotsTogetherAndZip {
  ...
  input:
    path plots
  output:
    path "sampleA-Samples-plots.tar.gz"
  script:
    """
    mkdir plots
    cp -L *.pdf plots
    tar -zcvf sampleA-Samples-plots.tar.gz plots
    """
